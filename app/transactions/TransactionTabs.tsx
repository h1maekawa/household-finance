'use client'
// 家計簿 — 実際に使ったお金を記録・確認する場所。
//
// 3つのタブに分ける:
//   取引     … 一覧と入力。日々いちばん開くのはここ
//   分析     … 収支グラフ・カテゴリ・支払方法別
//   確認待ち … カテゴリ未確定など、手を入れれば数字が正確になるもの
//
// 以前はグラフ・フィルタ・一覧が縦に全部並んでいて、取引を1件足すだけでも
// グラフを越えてスクロールする必要があった。
...
export default function TransactionTabs() {
  ...
  // 固定費/変動費の増減アラート。分析タブでだけ取りに行く
  const { data: analysis } = useSWR(
    tab === 'analysis'
      ? `/api/analysis/fixed-variable?year=${range.start.getFullYear()}&month=${range.start.getMonth() + 1}`
      : null,
    fetcher
  )
  ...
  // 確認待ち: カテゴリ未確定など、手を入れれば数字が正確になるもの
  const needsReview = allTransactions.filter(tx => tx.needs_review)
  ...
  return (
    <div className="mx-auto max-w-xl lg:max-w-4xl">
        ...
        {tab === 'review' && (
          needsReview.length === 0 ? (
            <div className="card p-6 text-center">
              <p className="text-sm font-bold">確認待ちはありません</p>
              <p className="mt-1 text-xs text-muted">
                取り込んだ取引はすべてカテゴリが確定しています。
              </p>
            </div>
          ) : (
            <>
              <div className="card border border-warning/30 bg-warning/5 p-4">
                <p className="text-sm font-bold">{needsReview.length}件のカテゴリが未確定です</p>
                <p className="mt-1 text-xs leading-relaxed text-muted">
                  未分類のままでも金額は「今月あと使える金額」に反映されますが、
                  カテゴリ別の内訳には出ません。確定すると使い道が正確になります。
                </p>
              </div>
              <TransactionList transactions={needsReview} onMutate={() => mutate()} />
            </>
          )
        )}
      </div>
      ...
      <button
        ...
        className="fixed bottom-[calc(env(safe-area-inset-bottom,0px)+4.5rem)] right-4 z-30 flex h-14 w-14 items-center justify-center rounded-full bg-primary text-2xl text-white shadow-lg transition-base active:scale-95 lg:bottom-6"
        aria-label="取引を追加"
      >
        ＋
      </button>
