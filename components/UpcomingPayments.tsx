...
// 「予定 → 支払い予定」タブの中身。
//
// もとは /cashflow という独立ページだった。マネープランと
// キャッシュフローはどちらも「これからのお金」を見る機能で、分かれていること自体が
// 重複感の原因だったため、/plan の内部タブとして統合した。
// 移設にあたって計算ロジックは一切変更していない。
...
export default function UpcomingPayments() {
  ...
  return (
    <div className="flex flex-col gap-4">
      {/* 流動資産。投資評価額は含めない(支払いに使えないため) */}
      <div className="card bg-primary p-4 text-white">
        <div>
          <p className="text-white/70 text-xs mb-1">流動資産（すぐ動かせるお金）</p>
          {editingBalance ? (
        ...
        <p className="mt-2 text-[11px] leading-relaxed text-white/60">
          投資評価額は含みません。支払いに使える預貯金だけで不足しないかを見ます。
        </p>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-4">
        ...
        {/* 固定費の登録・編集は同じページの「固定費」タブが持つ。
            ここは「これから出ていくお金を見る場所」なので、導線だけ残す。 */}
        <Link
          href="/plan?tab=fixed"
          scroll={false}
        ...
        >
          <span>
            <span className="font-medium">固定費を編集する</span>
            <span className="mt-0.5 block text-xs text-muted">家賃・サブスクなどの引き落とし予定</span>
          </span>
