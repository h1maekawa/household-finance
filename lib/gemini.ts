...
export async function explainFinance(question: string, context: string): Promise<string> {
  const model = genAI.getGenerativeModel({ model: 'gemini-1.5-flash' })
  const prompt = [
    'あなたは家計アプリ Flow+ の AI FP（パーソナル・ファイナンシャルプランナー）です。',
    '以下の「計算済みデータ」だけを根拠にユーザーの質問へ日本語で答えてください。',
    '',
    '厳守すること:',
    '- 金額・年数・月数は計算済みデータにある数字のみを使う。自分で計算し直さない',
    '- データに無い金額を推測して答えない。分からないときは「まだ計算できていません」と伝える',
    '- 新しい投資額・貯金額・削減額・副業収入額・利回り・目標額を独自に決めない',
    '- 特定の銘柄や商品の購入を勧めない。売買のタイミングを判断しない',
    '- 利回りは「仮定」として述べる。「確実に」「必ず」など保証と読める表現を使わない',
    '- 見直し候補は「削るべき」と断定せず、判断はユーザーに委ねる',
