 ...
 *
 * 'クレカ請求' だけを除外する。カード請求はカード利用そのものの再掲なので、
 * 含めると同じ支出を2回数えることになるため。
 *
 * '未分類' は除外しない。取り込み時のカテゴリは必ず '未分類' で入り
 * (app/api/transactions/import/route.ts)、ユーザーが確定するまでそのままなので、
 * 除外すると「実際には使っているのに消化額0円」になり、
 * 「あといくら使えるか」が実態より大きく出てしまう。
 * 分類が未確定なだけで、お金は確実に出ている。
 */
const ALWAYS_EXCLUDED_CATEGORIES = ['クレカ請求']

/** 分類が未確定の支出。金額には数えるが、カテゴリ別配分の学習には使わない */
