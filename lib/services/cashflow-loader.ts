// lib/services/cashflow-loader.ts
//
// キャッシュフロー予測の収集(I/O)。
//
// 以前は app/api/cashflow/route.ts の GET の中に直接書かれていた。Action Planner
// も「残高がマイナスになる日」と「カード請求に未割当の利用」を必要とするため、
// 同じパイプラインを2箇所へ写さないようここへ切り出した。
//
// **計算式は増やしていない。** lib/cashflow と fixed-cost-matching の既存関数を
// 呼ぶ順序をそのまま保っている。
...
export type CashflowLoad = Awaited<ReturnType<typeof loadCashflow>>

export async function loadCashflow(userId: string, supabase: SupabaseClient) {
  ...
  // 取得失敗を空配列として先へ流さない。予定が0件になると残高予測が実態より
  // 大きく出て、「不足しません」と誤って言うことになる
  for (const [label, res] of [
    ['account_balance', balanceRes],
    ['scheduled_payments', paymentsRes],
